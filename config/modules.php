<?php 


return [
    "B4T9Z"=>
        [
            
            "instructions_by"=>[
                "video",
                "Pictures"
            ],
            "video_url"=>"",
            "topics"=>[
                [
                    "title"=>"Moments when you become aware that you have type 2 diabetes for the first time",
                    "id"=>"A1B2C"
                ],
                [
                    "title"=>"Common feelings that are associated with a new diagnosis of type 2 diagnosis? (include real life stories in the updates)",
                    "id"=>"J3K5L"
                ],
                [
                    "title"=>"Effect of type 2 diabetes on daily life",
                    "id"=>"H5N7P"
                ],
                [
                    "title"=>"Long term effects of type 2 diabetes in life ",
                    "id"=>"Q2R8S"
                ],
                [
                    "title"=> "Common challenges and barriers to treatment and care",
                    "id"=>"W4T6U"
                ],
                [
                    "title"=> "Gaining a positive outlook to life",
                    "id"=>"Z1X9Y"
                ],
                [
                    "title"=> "Key important lifestyle changes needed to re-position to win",
                    "id"=>"A6C4E"
                ],
                [
                    "title"=>  "Key adjustments needed to re-position to  win (family, work, social life, travels)",
                    "id"=>"G2I4O"
                ],
                [
                    "title"=>"The four critical moments when it is important to re-position to win",
                    "id"=>"M5F7D"
                ],
                [
                    "title"=>"End of Module one",
                    "id"=>"ASF0L"
                ],
                
            ]
            
        ],

    "X3fZ9"=>
        [
            
            "instructions_by"=>[
                "video",
                "Pictures"
            ],
            "video_url"=>"",
            "topics"=>[
                [
                    "title"=>"Understanding type 2 diabetes",
                    "id"=>"KLSD0"
                ],
                [
                    "title"=>"What is diabetes?",
                    "id"=>"A2dF4"
                ],
                [
                    "title"=>"Learning the different types of diabetes",
                    "id"=>"J7kP5"
                ],
                [
                    "title"=>"What is type 1 diabetes?",
                    "id"=>"F8aT9"
                ],
                [
                    "title"=>"What is type 2 diabetes?",
                    "id"=>"Z4yD0"
                ],
                [
                    "title"=>"The risk factors for type 2 diabetes",
                    "id"=>"O2wV3"
                ],
                [
                    "title"=>"The progress from risk factors to full blown type 2 diabetes",
                    "id"=>"I6eL8"
                ],
                [
                    "title"=>"The signs and symptoms of type 2 diabetes ",
                    "id"=>"G1qS7"
                ],
                
                [
                    "title"=>"How to know that a person has type 2 diabetes",
                    "id"=>"C3oN4"
                ],
            ]
            
        ],

    "L0uV9"=>
        [
            
            "instructions_by"=>[
                "video",
                "Pictures"
            ],
            "video_url"=>"",
            "topics"=>[
                [
                    "title"=>"Introducing the effects of type 2 diabetes on the body",
                    "id"=>"VNS2b"
                ],
                [
                    "title"=>"What are the effects of type 2 diabetes on the body?",
                    "id"=>"P1rE2"
                ],
                [
                    "title"=>"Acute complications of type 2 diabetes",
                    "id"=>"S6tA3"
                ],
                [
                    "title"=>"Chronic complications of type 2 diabetes",
                    "id"=>"W4yG7"
                ],
                [
                    "title"=>"Conditions that increase the risk of having complications",
                    "id"=>"34POx"
                ],[
                    "title"=>"Preventing complications of type 2 diabetes",
                    "id"=>"Axc07"
                ],
                
            ]
            
        ],

    "F7hX8"=>
        [
            
            "instructions_by"=>[
                "video",
                "Pictures"
            ],
            "video_url"=>"",
            "topics"=>[
                [
                    "title"=>"Introduction to medications used for treating type 2 diabetes part 1",
                    "id"=>"Loxa3"
                ],
                [
                    "title"=>"Introduction to medications used for treating type 2 diabetes part 2",
                    "id"=>"B2cD3"
                ],
                [
                    "title"=>"Overview of blood glucose lowering medications for treating type 2 diabetes ",
                    "id"=>"H4fG5"
                ],
                [
                    "title"=>"Steps in treating type 2 diabetes",
                    "id"=>"U6vI7"
                ],
                [
                    "title"=>"Do supplements work?",
                    "id"=>"Q8sT9"
                ],
                [
                    "title"=>"Treatment targets for achieving blood sugar control",
                    "id"=>"A0jK1"
                ],
                
                
                
            ]
            
        ],

    "J2iK9"=>
        [
            
            "instructions_by"=>[
                "video",
                "Pictures"
            ],
            "video_url"=>"",
            "topics"=>[
                [
                    "title"=>"Introducing healthy and happy eating part 1",
                    "id"=>"10RTd"
                ],
                [
                    "title"=>"Introducing healthy and happy eating part 2",
                    "id"=>"Z2mN3"
                ],
                [
                    "title"=>"Understanding energy",
                    "id"=>"W4xO5"
                ],
                [
                    "title"=>"Set healthy and happy eating goals",
                    "id"=>"L6yP7"
                ],
                [
                    "title"=>"Clearing the hype about carbohydrates ",
                    "id"=>"F8gQ9"
                ],
                [
                    "title"=>"Clearing the hype about fats and oils",
                    "id"=>"R0hS1"
                ],
                [
                    "title"=>"Clearing the hype about proteins",
                    "id"=>"A3B5C"
                ],
                [
                    "title"=>"Making the right food choices",
                    "id"=>"1Z4Y6"
                ],
                [
                    "title"=>"Drinking and type 2 diabetes ",
                    "id"=>"W8R2Q"
                ],
                [
                    "title"=>"Healthy eating outside the home",
                    "id"=>"G6J9L"
                ],
                [
                    "title"=>"Tips for sustaining healthy eating habits",
                    "id"=>"0T5P7"
                ],
                    
                
            ]
            
        ],

    "O4pL6"=>
        [
            
            "instructions_by"=>[
                "video",
                "Pictures"
            ],
            "video_url"=>"",
           
            "topics"=>[
                [
                    "title"=>"Understanding physical activity",
                    "id"=>"M3N1X"
                ],
                [
                    "title"=>"What is physical activity ",
                    "id"=>"V3iU4"
                ],
                [
                    "title"=>"Types of physical activity ",
                    "id"=>"D5kE6"
                ],
                [
                    "title"=>"Benefits of physical activity",
                    "id"=>"M7nO8"
                ],
                [
                    "title"=>"How physical activity affects blood sugar",
                    "id"=>"C9oA0"
                ],
                [
                    "title"=>"How to start and sustain a physical activity routine",
                    "id"=>"T1pW2"
                ],
                [
                    "title"=>"Overcoming factors that hinder a regular physical activity routine",
                    "id"=>"F2H4J"
                ],
                
                    
                
            ]
            
        ],

    "C5uV0"=>
        [
            
            "instructions_by"=>[
                "video",
                "Pictures"
            ],
            "video_url"=>"",
            "extra_videos"=>[
               
                
            ],
            "topics"=>[
                [
                    "title"=>"Understand how to use medications for treating type 2 diabetes",
                    "id"=>"B3qX4"
                ],
                [
                    "title"=>"Important things to note when taking medications for type 2 diabetes",
                    "id"=>"Y5rF6"
                ],
                [
                    "title"=>"Learn how to take your medications correctly and consistently",
                    "id"=>"G7sH8"
                ],
                [
                    "title"=>"Start taking your medications correctly and consistently",
                    "id"=>"L1uZ2"
                ],
                
                
                    
                
            ]
            
        ],
    "T1wZ3"=>
        [
            
            "instructions_by"=>[
                "video",
                "Pictures"
            ],
            "video_url"=>"https://www.youtube.com/embed/276tcCCnLGo",
            "extra_videos"=>[
                
                
            ],
            "topics"=>[
                [
                    "title"=>"What are acute emergencies",
                    "id"=>"P2vM3"
                ],
                [
                    "title"=>"Understanding hypoglycaemia",
                    "id"=>"N4wO5"
                ],
                [
                    "title"=>"Understanding Hyperosmolar hyperglycaemic states",
                    "id"=>"Q6xR7"
                ],
                [
                    "title"=>"Understanding diabetes keto-acidosis ",
                    "id"=>"S8yT9"
                ],
                
                
                
                    
                
            ]
            
        ],

    "M9aY4"=>
        [
            
            "instructions_by"=>[
                "video",
                "Pictures"
            ],
            "video_url"=>"https://www.youtube.com/embed/Zpq3Eja6RQQ",
            "extra_videos"=>[
                
                
            ],
            "topics"=>[
                [
                    "title"=>"What are chronic complications",
                    "id"=>"A1zU2"
                ],
                [
                    "title"=>"Understanding eye complications ",
                    "id"=>"V3aI4"
                ],
                [
                    "title"=>"Understanding kidney complications",
                    "id"=>"D5bJ6"
                ],
                [
                    "title"=>"Understanding cardiovascular disease complications",
                    "id"=>"F7cK8"
                ],
                [
                    "title"=>"Understanding oral complications ",
                    "id"=>"H9dL0"
                ],
                [
                    "title"=>"understanding nerve related complications ",
                    "id"=>"X0eM1"
                ],
                [
                    "title"=>"Understanding foot complications",
                    "id"=>"Y2fN3"
                ],
                
                
                
                    
                
            ]
            
        ],

    "Z4gO5"=>
        [
            
            "instructions_by"=>[
                "video",
                "Pictures"
            ],
            "video_url"=>"https://www.youtube.com/embed/-jz0ShHCTJQ",
            "extra_videos"=>[
                
                
            ],
            "topics"=>[
                [
                    "title"=>"Good reasons for staying on the winning lane",
                    "id"=>"P6hR7"
                ],
                [
                    "title"=>"Navigating the rough and bumpy lane of managing type 2 diabetes",
                    "id"=>"T8iU9"
                ],
                [
                    "title"=>"Coping with the everyday turns and twists on the lane
                    ",
                    "id"=>"C0jV1"
                ],
                [
                    "title"=>"Staying on the lane when life happens",
                    "id"=>"W2kX3"
                ],
                
                
                
                
                    
                
            ]
            
        ],

];